...
public sealed class Explorer(
    ...
    IOptions<ExplorerOptions> explorerOptions) : IExplorer
{
    ...
    private IEnumerable<FileEntity> SkipLinuxLogicalPartitions(FileEntity[] drives)
    {
        foreach (var drive in drives)
            if (Directory.Exists(drive.Path))
                ...
                logger.LogWarning(
                    "{path} is not existing logical drive, maybe it's linux  partition and user has dual boot with linux distro",
                    drive.Path);
    }
}